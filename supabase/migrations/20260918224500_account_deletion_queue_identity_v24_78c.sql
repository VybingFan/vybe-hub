-- VYBE V24.78C
-- Make immediate account-deletion Work Queue items identify the account.
...
SET
  title = concat('Immediate account deletion: ', COALESCE(au.email, adr.user_id::text)),
  description = concat(
    'Account holder ', COALESCE(au.email, adr.user_id::text),
    ' asked for immediate deletion during the 7-day cancellation period.'
  ),
  updated_at = now()
