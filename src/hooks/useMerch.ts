...
export function useMerch(creatorId?: string) {
  return useQuery({
    queryKey: ["merch", creatorId],
    queryFn: () => merchService.list(creatorId!),
    enabled: !!creatorId,
  });
}
...
export function useDeleteMerch(creatorId?: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: merchService.remove,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["merch", creatorId] }),
  });
}
