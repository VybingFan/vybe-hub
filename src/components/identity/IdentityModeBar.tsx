...
export function IdentityModeBar(){
 ...
 useEffect(()=>{void(async()=>{const{data,error}=await(supabase.rpc as any)("ensure_my_identities");if(!error&&data){setItems(data);const saved=getActiveIdentity();const resolved=data.find((x:Identity)=>x.id===saved?.id)||data.find((x:Identity)=>x.identity_type==="supporter")||data[0]||null;setActive(resolved);if(resolved){localStorage.setItem(ACTIVE_IDENTITY_KEY,JSON.stringify(resolved));await(supabase.rpc as any)("set_my_active_identity",{p_identity_id:resolved.id});window.dispatchEvent(new CustomEvent("vybe:identity-changed",{detail:resolved}))}}})()},[]);
 ...
 const beginCreatorStudioActivation=()=>{navigate({to:"/auth/onboarding",search:{role:"creator"}})};
 ...
 return <div className="border-b border-primary/15 bg-gradient-to-r from-primary/8 via-background to-violet-500/8"><div className="mx-auto flex min-h-12 max-w-7xl flex-wrap items-center gap-2 px-4 py-2"><div className="mr-auto flex min-w-0 items-center gap-2"><span className="truncate text-sm text-muted-foreground">Viewing as <strong className="text-foreground">{active.display_name}</strong></span><Badge variant="outline" className="capitalize">{active.identity_type}</Badge><Badge className="hidden capitalize sm:inline-flex">{workspace === "creator_studio" ? "Creator HQ" : workspace.replaceAll("_"," ")}</Badge></div>{items.length>1?<select aria-label="Choose active identity" value={active.id} onChange={e=>void choose(e.target.value)} className="h-9 max-w-48 rounded-full border border-border bg-background px-3 text-sm shadow-sm">{items.map(x=><option key={x.id} value={x.id}>{x.identity_type==="supporter"?"Supporter Mode":x.display_name}</option>)}</select>:null}{!hasRole("creator")&&hasRole("supporter")?<Button size="sm" className="rounded-full bg-gradient-brand text-white" onClick={beginCreatorStudioActivation}><LayoutDashboard className="mr-2 h-4 w-4"/>Become a Creator</Button>:null}{(hasRole("creator")||hasRole("admin"))&&workspace!=="creator_studio"?<Button size="sm" className="rounded-full bg-gradient-brand text-white" onClick={()=>void openWorkspace("creator_studio")}><LayoutDashboard className="mr-2 h-4 w-4"/>Creator HQ</Button>:null}{hasRole("business")&&workspace!=="business_studio"?<Button size="sm" variant="outline" className="rounded-full" onClick={()=>void openWorkspace("business_studio")}><BriefcaseBusiness className="mr-2 h-4 w-4"/>Business Studio</Button>:null}{workspace!=="member"&&creatorCanBrowse?<Button size="sm" variant="outline" className="rounded-full" onClick={()=>void openWorkspace("member")}><Compass className="mr-2 h-4 w-4"/>Explore VYBE</Button>:null}{workspace==="member"&&active.identity_type!=="supporter"?<Button size="sm" variant="outline" className="rounded-full" onClick={()=>void openWorkspace("member")}><Users className="mr-2 h-4 w-4"/>Supporter Mode</Button>:null}</div></div>;
}
