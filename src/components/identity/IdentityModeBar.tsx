...
import { BriefcaseBusiness,Compass,LayoutDashboard,ShieldCheck,Users } from "lucide-react";
...
export function IdentityModeBar(){
 ...
 const openWorkspace=async(mode:WorkspaceMode)=>{window.sessionStorage.setItem(ACTIVE_WORKSPACE_KEY,mode);if(mode==="creator_studio"){const creator=items.find(x=>x.identity_type==="creator");if(creator)await choose(creator.id);navigate({to:"/dashboard"});return}if(mode==="business_studio"){const business=items.find(x=>x.identity_type==="business");if(business)await choose(business.id);navigate({to:"/business"});return}if(mode==="member"){const supporter=items.find(x=>x.identity_type==="supporter");if(supporter)await choose(supporter.id);navigate({to:"/home"});return}navigate({to:"/admin"})};
