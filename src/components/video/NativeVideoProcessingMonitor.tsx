...
export function NativeVideoProcessingMonitor({creatorId,videos}:{creatorId?:string;videos:CreatorVideo[]}){
 ...
 if(!processing.length)return null;return <div className="flex items-center gap-2 rounded-xl border border-cyan-400/30 bg-cyan-400/5 px-4 py-3 text-sm"><Loader2 className="h-4 w-4 animate-spin text-cyan-500"/><span>{processing.length} video{processing.length===1?" is":"s are"} being processed. VYBE is preparing {processing.length===1?"this video":"these videos"} for playback. You can publish {processing.length===1?"it":"them"} when processing is complete.</span></div>;
}
