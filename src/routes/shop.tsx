...
import { ArrowRight, ExternalLink, Loader2, Search, ShoppingBag, Store } from "lucide-react";
...
export const Route = createFileRoute("/shop")({ component: PublicMerchPage });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        <section className="border-b border-border/60 bg-gradient-hero">
          <div className="mx-auto max-w-7xl px-5 py-9 sm:px-6 sm:py-16 md:py-20">
            ...
            <h1 className="mt-4 max-w-4xl text-3xl font-bold tracking-tight sm:mt-5 sm:text-4xl md:text-6xl">
              Support creators beyond the stream.
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground sm:mt-5 sm:text-lg sm:leading-8">
              Browse active merchandise, collectibles, art, accessories, and limited creator drops
              from across VYBE.
            </p>
          </div>
        </section>

        <section className="mx-auto max-w-7xl px-5 py-8 sm:px-6 sm:py-14">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-medium text-primary">Creator marketplace</p>
            ...
            <div className="relative w-full sm:max-w-sm">
              ...
              <Input
                ...
                placeholder="Search merch or creator"
          ...
          {!loading && failed ? (
            <div className="mt-6 rounded-2xl border border-dashed border-border bg-card p-6 text-center">
              ...
              <h3 className="mt-3 text-lg font-semibold">Merchandise could not be loaded.</h3>
          ...
          {!loading && !failed && !items.length ? (
            <div className="mt-6 rounded-2xl border border-dashed border-border bg-card p-6 text-center">
              <Store className="mx-auto h-8 w-8 text-muted-foreground" />
              <h3 className="mt-3 text-lg font-semibold">Creator merch is coming in.</h3>
              <p className="mx-auto mt-1 max-w-lg text-sm leading-6 text-muted-foreground">
                Active creator products will appear here as creators publish them.
              </p>
              <Button asChild variant="outline" className="mt-4 rounded-full">
                <Link to="/explore" search={{ q: "" }}>Discover creators</Link>
              </Button>
            </div>
          ) : null}
          ...
          {!loading && !failed && items.length && !visible.length ? (
            <div className="mt-6 rounded-2xl border border-dashed border-border bg-card p-6 text-center">
              <p className="font-semibold">No merch matches this search.</p>
              <button
                type="button"
          ...
          {!loading && !failed && visible.length ? (
            <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-8 sm:gap-5 lg:grid-cols-4">
              {visible.map((item) => {
                ...
                const canBuy =
                  item.availability === "available_externally" && Boolean(item.purchase_url);

                return (
                  <article
                    key={item.id}
                    className="flex min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-card sm:rounded-3xl"
                  >
                    ...
                    <div className="flex flex-1 flex-col p-3 sm:p-5">
                      <div className="flex min-w-0 items-start justify-between gap-2">
                        <Badge variant="outline" className="max-w-[7rem] truncate px-2 text-[9px] sm:max-w-none sm:text-xs">
                          {item.category}
                        </Badge>
                        {price ? <p className="shrink-0 text-sm font-semibold sm:text-base">{price}</p> : null}
                      </div>

                      <h3 className="mt-2 line-clamp-2 text-sm font-semibold sm:mt-3 sm:text-lg">
                        {item.title}
                      </h3>

                      {item.creator_username ? (
                        <a
                          href={`/artist/${item.creator_username}`}
                          className="mt-1 truncate text-xs font-medium text-primary sm:text-sm"
                        >
                          {item.creator_name}
                        </a>
                      ) : (
                      ...
                      <p className="mt-2 line-clamp-2 flex-1 text-xs leading-5 text-muted-foreground sm:text-sm sm:leading-6">
                        {item.description}
                      </p>

                      <p className="mt-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground sm:mt-3 sm:text-xs">
                        {availabilityLabel(item.availability)}
                      </p>

                      {canBuy ? (
                        <Button asChild size="sm" className="mt-3 w-full rounded-full bg-gradient-brand sm:mt-4">
                          <a href={item.purchase_url ?? "#"} target="_blank" rel="noreferrer">
                            Shop now <ExternalLink className="ml-2 h-3.5 w-3.5" />
                          </a>
                        </Button>
                      ) : item.creator_username ? (
                        <Button asChild size="sm" variant="outline" className="mt-3 w-full rounded-full sm:mt-4">
                          <a href={`/artist/${item.creator_username}`}>
                            View creator <ArrowRight className="ml-2 h-3.5 w-3.5" />
                          </a>
                        </Button>
                      ) : (
                        <Button size="sm" variant="outline" className="mt-3 w-full rounded-full sm:mt-4" disabled>
                          {availabilityLabel(item.availability)}
                        </Button>
                      )}
                    </div>
