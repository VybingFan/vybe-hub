...
function RightsReviewPage(){
 ...
 return <div className="mx-auto max-w-6xl space-y-7">
  ...
  <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
   <RightsModule icon={LibraryBig} title="Rights Registry" description="Review each work's rights declaration, creator certification, supporting documents, and fingerprint status." status={`${stats.pendingDocuments} documents pending`} to="/admin/rights-registry" action="Open rights registry"/>
   <RightsModule icon={Fingerprint} title="Fingerprints & Matches" description="Monitor processing jobs and match candidates. The processor itself is activated separately after controlled testing." status={`${jobCounts.queued??0} queued · ${stats.matchCandidates} matches`} tab="processing" action="Review processing"/>
   <RightsModule icon={FileCheck2} title="Commerce Rights Review" description="Preserve the existing specialized review of creator rights declarations before music products can be activated for sale." status="Existing workflow" to="/admin/commerce-rights" action="Open commerce review"/>
   <RightsModule icon={Copyright} title="DMCA & Copyright Operations" description="Formal copyright cases, takedowns, counter-notices, strikes, restoration, legal holds, and rights audits." status={`${openCopyright} open reports`} to="/admin/copyright" action="Open copyright operations"/>
  </section>
  <Card><CardHeader><CardTitle className="text-lg">Rights system roadmap</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-3"><RoadmapItem icon={UsersRound} title="Permissions & Collaborations" description="Planned: document authorized duplicates, collaborator roles, licenses, and creator-to-creator permission scope without treating every fingerprint match as infringement."/><RoadmapItem icon={FileCheck2} title="Copyright Registration" description="Planned: creator priority lists, readiness records, document preparation, and optional VYBE-assisted federal registration workflow."/><RoadmapItem icon={ShieldCheck} title="Enhanced Rights Record" description={`Current rights-document review queue: ${stats.pendingDocuments}. Future Rights Vault work will build from the existing rights-document foundation.`}/></CardContent></Card>
  {loading?<p className="text-muted-foreground">Loading rights records…</p>:null}{!loading?<Tabs defaultValue="processing"><div id="rights-tabs" className="scroll-mt-24"><TabsList className="grid h-auto w-full grid-cols-1 gap-1 p-1 sm:grid-cols-3"><TabsTrigger value="processing" className="min-h-10 whitespace-normal px-3 py-2 text-center leading-5">Fingerprint processing</TabsTrigger><TabsTrigger value="cases" className="min-h-10 whitespace-normal px-3 py-2 text-center leading-5">Rights review cases</TabsTrigger><TabsTrigger value="reports" className="min-h-10 whitespace-normal px-3 py-2 text-center leading-5">Incoming copyright reports</TabsTrigger></TabsList><TabsContent value="processing" className="mt-6 space-y-5"><ProcessingOverview jobs={jobs}/></TabsContent><TabsContent value="cases" className="mt-6 space-y-5">{cases.length?cases.map((x)=><CaseCard key={x.id} item={x}/>):<EmptyState text="No automated or manual review cases are open."/>}</TabsContent><TabsContent value="reports" className="mt-6 space-y-5"><Card className="border-primary/20 bg-primary/5"><CardContent className="flex flex-col justify-between gap-3 p-4 sm:flex-row sm:items-center"><p className="text-sm text-muted-foreground">This tab preserves the existing intake review. Full legal operations remain in the dedicated DMCA & Copyright workspace.</p><Button asChild size="sm" variant="outline"><Link to="/admin/copyright">Open legal operations <ArrowRight className="ml-2 h-4 w-4"/></Link></Button></CardContent></Card>{reports.length?reports.map((r)=><ReportCard key={r.id} report={r} onSave={updateReport}/>):<EmptyState text="No copyright reports have been submitted."/>}</TabsContent></div></Tabs>:null}
 </div>;
}

function RightsModule({icon:Icon,title,description,status,to,tab,action}:{icon:typeof ShieldCheck;title:string;description:string;status:string;to?:"/admin/rights-registry"|"/admin/commerce-rights"|"/admin/copyright";tab?:"processing";action:string;}){return <Card className="flex h-full flex-col"><CardContent className="flex h-full flex-col p-5"><div className="flex items-start justify-between gap-3"><span className="rounded-xl bg-primary/10 p-2 text-primary"><Icon className="h-5 w-5"/></span><Badge variant="secondary">{status}</Badge></div><p className="mt-4 font-semibold">{title}</p><p className="mt-2 flex-1 text-sm leading-6 text-muted-foreground">{description}</p>{to?<Button asChild variant="outline" className="mt-5"><Link to={to}>{action}<ArrowRight className="ml-2 h-4 w-4"/></Link></Button>:<Button variant="outline" className="mt-5" onClick={()=>document.querySelector(`[data-value=\"${tab}\"]`)?.dispatchEvent(new MouseEvent("click",{bubbles:true}))}>{action}<ArrowRight className="ml-2 h-4 w-4"/></Button>}</CardContent></Card>;}
function RoadmapItem({icon:Icon,title,description}:{icon:typeof ShieldCheck;title:string;description:string;}){return <div className="rounded-xl border p-4"><div className="flex items-center gap-2"><Icon className="h-4 w-4 text-primary"/><p className="font-medium">{title}</p><Badge variant="outline" className="ml-auto">Planned</Badge></div><p className="mt-2 text-sm leading-6 text-muted-foreground">{description}</p></div>;}
function ProcessingOverview({jobs}:{jobs:ProcessingJob[]}){const counts=jobs.reduce<Record<string,number>>((r,j)=>{r[j.status]=(r[j.status]??0)+1;return r;},{});const active=jobs.some((j)=>j.processor_version&&["processing","completed","flagged"].includes(j.status));return <><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Queued" value={counts.queued??0} icon={Activity}/><Metric label="Completed" value={counts.completed??0} icon={CheckCircle2}/><Metric label="Flagged" value={counts.flagged??0} icon={AlertTriangle}/><Metric label="Failed" value={counts.failed??0} icon={ShieldCheck}/></div>{!active?<Card className="border-amber-500/40 bg-amber-500/5"><CardContent className="p-5 text-sm leading-6"><p className="font-medium">Native rights processor not yet observed</p><p className="text-muted-foreground">Uploads can queue safely, but FFmpeg and Chromaprint results will appear only after the separate private processor is deployed and the controlled test sequence is approved.</p></CardContent></Card>:null}{jobs.length?jobs.map((j)=><Card key={j.id}><CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-medium">{j.tracks?.title??"Unavailable track"}{j.tracks?.primary_artist_name?` · ${j.tracks.primary_artist_name}`:""}</p><p className="mt-1 text-xs text-muted-foreground">Queued {new Date(j.queued_at).toLocaleString()} · Attempt {j.attempt_count}{j.processor_version?` · ${j.processor_version}`:""}</p>{j.last_error?<p className="mt-2 max-w-2xl text-sm text-destructive">{j.last_error}</p>:null}</div><StatusBadge status={j.status}/></CardContent></Card>):<EmptyState text="No audio-processing jobs exist yet."/>}</>;}
function CaseCard({item}:{item:ModerationCase}){return <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle className="text-lg">{item.tracks?.title??"Rights review"} {item.tracks?.primary_artist_name?`· ${item.tracks.primary_artist_name}`:""}</CardTitle><div className="flex gap-2"><Badge variant="outline">{item.severity}</Badge><StatusBadge status={item.status}/></div></div></CardHeader><CardContent className="space-y-3 text-sm"><p>{item.summary||"No summary has been entered."}</p><p className="text-muted-foreground">{item.case_type.replaceAll("_"," ")}{item.risk_score===null?"":` · Risk signal ${Math.round(item.risk_score*100)}%`} · {new Date(item.created_at).toLocaleString()}</p>{item.reason_codes.length?<div className="flex flex-wrap gap-2">{item.reason_codes.map((r)=><Badge key={r} variant="secondary">{r.replaceAll("_"," ")}</Badge>)}</div>:null}</CardContent></Card>;}
function Metric({label,value,icon:Icon,to,tab}:{label:string;value:number;icon:typeof Activity;to?:"/admin/rights-registry"|"/admin/copyright";tab?:"processing"|"cases";}){const content=<CardContent className="flex items-center gap-3 p-5"><Icon className="h-5 w-5 text-primary"/><div className="min-w-0"><p className="text-2xl font-semibold">{value}</p><p className="text-xs text-muted-foreground">{label}</p></div>{to||tab?<ArrowRight className="ml-auto h-4 w-4 shrink-0 text-muted-foreground"/>:null}</CardContent>;if(to)return <Link to={to} className="block rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><Card className="h-full cursor-pointer transition hover:border-primary/40 hover:shadow-sm">{content}</Card></Link>;if(tab)return <button type="button" className="block w-full text-left rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={()=>{document.querySelector(`[data-value=\"${tab}\"]`)?.dispatchEvent(new MouseEvent("click",{bubbles:true}));window.requestAnimationFrame(()=>document.getElementById("rights-tabs")?.scrollIntoView({behavior:"smooth",block:"start"}));}}><Card className="h-full cursor-pointer transition hover:border-primary/40 hover:shadow-sm">{content}</Card></button>;return <Card>{content}</Card>;}
