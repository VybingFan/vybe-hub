...
function Overview({ data, readiness, onOpen }: { data: CreatorEpkWorkspace; readiness: ReturnType<typeof calculateReadiness>; onOpen: (tab: Tab) => void }) {
  return (
    <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr] min-[900px]:gap-4">
      <Card><CardHeader><CardTitle>EPK readiness checklist</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">
        {readiness.items.map((item) => (
          <button key={item.label} type="button" onClick={() => onOpen(item.tab)} className="flex items-start gap-3 rounded-xl border border-border/70 p-4 text-left transition hover:border-primary/50 min-[900px]:p-3">
