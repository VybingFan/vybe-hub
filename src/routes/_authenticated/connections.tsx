...
import { Mail, MessageCircle, Search, Star, UserRound } from "lucide-react";
...
function ConnectionCard({ connection, creatorId }: { connection: ListenerConnection; creatorId?: string }) {
  ...
  return <Card className={favorite ? "border-primary/40" : ""}><CardContent className="space-y-5 p-5">
    <div className="flex items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 font-semibold"><UserRound className="h-4 w-4" />{connection.display_name || "Listener"}</h2><p className="mt-1 text-sm text-muted-foreground">via {connection.playlists?.title || "shared playlist"}</p></div><Button size="icon" variant="ghost" onClick={() => setFavorite((value) => !value)} aria-label="Favorite connection"><Star className={`h-5 w-5 ${favorite ? "fill-amber-400 text-amber-400" : ""}`} /></Button></div>
