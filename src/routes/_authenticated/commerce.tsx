...
function CommerceStudio() {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      ...
      <section className="space-y-3"><h2 className="text-xl font-semibold">Prepared products</h2>{products.length ? products.map((product) => <Card key={product.id}><CardContent className="flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-medium">{product.title}</p><p className="text-sm text-muted-foreground">{product.product_type} · ${(product.price_cents / 100).toFixed(2)} · {product.fulfillment}</p></div><div className="flex items-center gap-2"><Badge variant={product.status === "active" ? "default" : "outline"}>{product.status}</Badge>{product.status === "draft" ? <Button size="sm" onClick={() => changeStatus.mutate({ id: product.id, status: "active" })}>Publish listing</Button> : product.status === "active" ? <Button size="sm" variant="outline" onClick={() => changeStatus.mutate({ id: product.id, status: "retired" })}>Remove from sale</Button> : null}</div></CardContent></Card>) : <Card><CardContent className="p-6 text-sm text-muted-foreground">No sales listings prepared yet.</CardContent></Card>}</section>
    </div>
  </div>;
}
