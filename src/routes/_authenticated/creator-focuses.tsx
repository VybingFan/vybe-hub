  ...
  return <div className="mx-auto max-w-6xl space-y-7">
    ...
    <section className="space-y-3">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {access.map((item) => <Card key={item.focus_code}><CardContent className="space-y-4 p-5">
...
function Pricing({ planCode, founding }: { planCode: string; founding: boolean }) {
  ...
  return <Card><CardContent className="space-y-4 p-6"><div><h2 className="text-xl font-semibold">Focus subscription options</h2><p className="mt-1 text-sm text-muted-foreground">Checkout is not open yet. These options are shown now so testing and pricing remain clear.</p></div><div className="grid gap-3 md:grid-cols-2"><div className="rounded-xl border p-4"><p className="font-semibold">Second Creator Focus</p><p className="mt-1 text-sm">${CREATOR_FOCUS_ADD_ON_PRICING.secondFocus.monthly}/month or ${CREATOR_FOCUS_ADD_ON_PRICING.secondFocus.annual}/year</p><p className="mt-2 text-xs text-muted-foreground">Available to Plus, Pro, and Studio. Adds one workspace.</p></div>{pro || studio ? <div className="rounded-xl border p-4"><p className="font-semibold">{studio ? "Studio" : "Pro"} Multi-Focus</p><p className="mt-1 text-sm">${studio ? CREATOR_FOCUS_ADD_ON_PRICING.studioMultiFocus.monthly : CREATOR_FOCUS_ADD_ON_PRICING.proMultiFocus.monthly}/month · up to five</p><p className="mt-2 text-xs text-muted-foreground">Supports the higher storage, analytics, moderation, and usage needs of three or more workspaces.</p></div> : null}</div>{!paid ? <p className="text-sm text-muted-foreground">Creator Free includes one focus. Upgrade the base membership before adding another.</p> : null}</CardContent></Card>;
}
